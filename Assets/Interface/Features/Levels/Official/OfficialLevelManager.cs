...
public class OfficialLevelManager {
    ...
    // the Task.FromResult stayed from the BaseLevelManager abstraction but is left
    // as it allows for non-blocking loads (i think)
    public Task<List<LevelMetadata>> LazyLoadLevels() {
        EnsureCache();
        return Task.FromResult(_levelMetadata);
    }

    public Task<Level> LoadLevel(int id) {
        EnsureCache();
